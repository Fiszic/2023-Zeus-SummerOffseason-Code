...
public class SwerveSubsystem extends SubsystemBase{
    ...
    public final SwerveModule frontLeft = new SwerveModule(
        Constants.ID.M1Drive, Constants.ID.M1Rot , 
        false, false, Constants.ID.M1Enc, 
        Constants.ID.M1Offset, true);

    private final SwerveModule frontRight = new SwerveModule(
        Constants.ID.M2Drive, Constants.ID.M2Rot , 
        false, false, Constants.ID.M2Enc, 
        Constants.ID.M2Offset, true);

    private final SwerveModule backLeft = new SwerveModule(
        Constants.ID.M3Drive, Constants.ID.M3Rot , 
        false, false, Constants.ID.M3Enc, 
        Constants.ID.M3Offset, true);

    private final SwerveModule backRight = new SwerveModule(
        Constants.ID.M4Drive, Constants.ID.M4Rot , 
        false, false, Constants.ID.M4Enc, 
        Constants.ID.M4Offset, true);
    ...
    public void update(){
        try{
        if(Constants.activeVariables.AprilTag&&aprilLoc().length!=0&&aprilLoc()[0]!=0&&aprilLoc()[1]!=0)
        if(Constants.activeVariables.isRed)
    ...
    public void setModuleStates(SwerveModuleState[]desiredStates){
        SwerveDriveKinematics.desaturateWheelSpeeds(desiredStates, Constants.RobotInfo.MAX_ROBOT_SPEED);
    ...
    public void setReflectiveMode(){
        ...
        Constants.activeVariables.AprilTag = false;
    }

    public void setAprilMode(){
        var table =NetworkTableInstance.getDefault().getTable("limelight-slhs");
        table.getEntry("pipeline").setNumber(0);
        Constants.activeVariables.AprilTag = true;
    }
