...
public class CubePlacement extends CommandBase{
    ...
    public void execute() {
        if(!m_claw.getState()){
            m_ele.setMotorPos(Constants.SetPoints.MSElevator);
            m_arm.setMotorPos(Constants.SetPoints.MSArm);
            m_wrist.setMotorPos(Constants.SetPoints.MSWrist);
            m_tele.setState(Constants.SetPoints.MSExtension);
            if(m_arm.getEncoderPos()>Constants.SetPoints.MSArm){
                m_claw.open();
                m_claw.setSpeed(0.1);
            }
        }else{
